...
export const renderLevels = (data: any[], handleClose: () => void) => {
  return data.map((item: any, index: number) => {
    ...
    if (item.extLink) {
      return (
        <Typography variant="h6" sx={{ py: 1 }} key={index}>
          <NavLink style={{color: 'inherit'}} href={item.url}>{item.title}</NavLink>
        </Typography>
      );
    }
    ...
    if (item.title === 'Home') {
      return (
        <Box key={index} py={1}>
        <NavLink style={{color: 'inherit'}} href={item.url || '/'} onClick={handleClose}>
          {item.title}
        </NavLink>
      </Box>
      );
